...
router.get('/', function(req, res, next) {
	sql.pool.query('SELECT * FROM '+sql.tables.Gestori, function(err, data) {
...
router.get('/:id', function(req, res, next) {
	sql.pool.query('SELECT * FROM '+sql.tables.Gestori+' WHERE id=$1', [req.params.id], function(err, data) {
		if (err) rest.error500(res, err);
...
router.delete('/:id', function(req, res, next) {
	sql.pool.query('DELETE FROM '+sql.tables.Gestori+' WHERE id=$1', [req.params.id], function(err, data) {
		if (err) rest.error500(res, err);
